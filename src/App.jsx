...
import CallActivityReportsPage from "./components/CallActivityReportsPage";
...
const LIVE_ATTENDANCE_TRIGGER_MINUTES = Array.from({ length: 12 }, (_, i) => i * 5); // every 5 minutes, on the clock
...
const PAGE_HEADER_TITLES = {
  ...
  call_reports: "Call Activity Reports",
...
export default function App() {
  ...
  const [employeeDashboardView, setEmployeeDashboardView] = useState("dashboard");
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeAttendanceSettings(
      ...
      () => {
        // offline/permission error - local fallback already applied by the service
      }
    );
    ...
    <div className="app-shell">
      ...
      <div className="secondheader">
            ...
            {activePage === "employee_dashboard" && canViewEmployeeCallActivityLog ? (
              <div className="portal-topbar-switch" role="tablist" aria-label="My Workspace views">
                <button
                  ...
                  onClick={() => setEmployeeDashboardView("dashboard")}
                  role="tab"
                ...
                <button
                  type="button"
                  className={`portal-topbar-switch-btn ${employeeDashboardView === "call_activity" ? "active" : ""}`}
                  onClick={() => setEmployeeDashboardView("call_activity")}
                  role="tab"
                  aria-selected={employeeDashboardView === "call_activity"}
                >
                  Call / Activity Log
                </button>
              </div>
        ...
        >
              ...
              {activePage === "call_reports" && (
                <div className="portal-page-pad">
                  <CallActivityReportsPage
                    viewer={user}
                    employees={allEmployeesForSharedPages}
                  />
                </div>
              ...
              {activePage === "employee_dashboard" && (
                <div className="portal-page-pad">
                  <EmployeeDashboard
                    ...
                    loadingTodayLogs={loadingTodayLogs}
                    loadingAssignments={loadingAssignments}
                    assignmentsError={assignmentsError}
                    nowMs={nowMs}
                    onScheduleReached={() => setNowMs(Date.now())}
                    endDate={endDate}
                    businessTimeZone={businessTimeZone}
                    onFetchFullHistory={fetchFullHistoryForUser}
                    historyByUserId={historyByUserId}
